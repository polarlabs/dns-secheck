...
use crate::constants::{DNS_SECHECK_KEY_LENGTH_USIZE, DNS_SECHECK_MAX_UDP_MESSAGE_SIZE};
...
pub async fn run(socket: TokioUdpSocket, cache: Cache) -> std::io::Result<()> {
    ...
    loop {
        ...
        if is_dns_packet(packet) {
            handle_dns(packet, addr, &socket, cache.clone()).await?;
        } else {
...
async fn handle_dns(
    ...
    match key {
        None => {}
        Some(key) => {
            let message = dns_protocol::build_dns_response(&message);
